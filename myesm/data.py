...
class Alphabet(object):
    ...
    def get_traindata_converter(self):
        return PreConverter(self)
...
class PreConverter(object):
    ...
    def lengthNormalization(self, x, corp_length, start_index):
        Length = corp_length
        x = x[start_index:start_index+corp_length]
        return x
    ...
    def __call__(self, raw_batch: Sequence[Tuple[str, str]]):
        ...
        # Make sure each sequence has the same length
        for seq_str in seq_str_list:
            tmp_array = seq_str
            if len(seq_str) > max_len:
                n = len(seq_str) - max_len
                x = int(random.uniform(0, n))
                #start_index = int(random.uniform(1, n - x + 1)) #alphafold
                start_index = 0
                tmp_array = self.lengthNormalization(seq_str, max_len, start_index)

            tmp_array = self.alphabet.encode(tmp_array)
            cand_maked_pos = [i for i, token_idx in enumerate(tmp_array)
                          if token_idx in self.alphabet.standard_idx]
            random.shuffle(cand_maked_pos)
            # 15% of tokens are randomly sampled from the sequence
            n_pred = int(len(tmp_array) * 0.15)
            #masked_tokens, masked_pos = [], []
            masked_tokens = [0  for i in range(max_len + int(self.alphabet.prepend_bos) + int(self.alphabet.append_eos))]
            masked_pos = [0  for i in range(max_len + int(self.alphabet.prepend_bos) + int(self.alphabet.append_eos))]
            for pos in cand_maked_pos[:n_pred] :
                #masked_pos.append(pos)
                #masked_tokens.append(tmp_array[pos])
                masked_pos[pos + int(self.alphabet.prepend_bos)] = 1
                masked_tokens[pos + int(self.alphabet.prepend_bos)] = tmp_array[pos]
                r = random.random() 
                if r < 0.8:
                    tmp_array[pos] = self.alphabet.tok_to_idx["<mask>"]
                elif r > 0.9:
                    index = random.randint(0, len(self.alphabet.standard_toks) - 1)
                    replace_token = self.alphabet.standard_toks[index]
                    tmp_array[pos] = self.alphabet.tok_to_idx[replace_token]

            seq_encoded_list.append(tmp_array)
        ...
        for i, (label, seq_str, seq_encoded) in enumerate(
            ...
            if self.alphabet.append_eos:
                tokens[i, len(seq_encoded) + int(self.alphabet.prepend_bos)] = self.alphabet.eos_idx
            #print('myesm: ', len(seq_encoded), self.alphabet.cls_idx, self.alphabet.eos_idx)

        return masked_tokens_list, masked_pos_list, tokens
